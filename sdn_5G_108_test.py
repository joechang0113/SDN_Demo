...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        bands = [parser.OFPMeterBandDrop(int(6000))]
        ...
        bands1 = [parser.OFPMeterBandDrop(int(200000))]
        ...
        # ------------------------ZyXEL_ap > ap1_Blue , ap1_Yellow > port10
        # ------------------------port8 > asus_ap2
        # ------------------------port4 > Efence_server
        # ------------------------port2 > Efence_camera

        # # port2 to Efence_camera
        ...
        self.add_meter_flow(datapath, 2, match, actions, 2)
        ...
        # # laptop(build others flow) to port12
        # match = parser.OFPMatch(in_port=12,
    ...
    def _packet_in_handler(self, ev):
        ...
        #     bands2 = [parser.OFPMeterBandDrop(int(5000))]
        #     mod2 = parser.OFPMeterMod(datapath=datapath,
        #                               command=ofproto.OFPMC_MODIFY,
        #                               flags=ofproto.OFPMF_KBPS,
        #                               meter_id=int(2),
        ...
        #     bands3 = [parser.OFPMeterBandDrop(int(400))]
        #     mod3 = parser.OFPMeterMod(datapath=datapath,
        #                               command=ofproto.OFPMC_MODIFY,
        #                               flags=ofproto.OFPMF_KBPS,
        #                               meter_id=int(2),
        #                               bands=bands3)
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            self.logger.info(
                ...
                stat.rx_bytes / 1024,
                stat.tx_bytes / 1024,
                int(stat.rx_bytes / 1024 / 5),
            )

            if stat.port_no == 2:
                pktport2 = (stat.rx_bytes / 1024 / 5)
                pktport2tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 4:
                pktport4 = (stat.rx_bytes / 1024 / 5)
                pktport4tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 6:
                pktport6 = (stat.rx_bytes / 1024 / 5)
                pktport6tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 7:
                pktport7 = (stat.rx_bytes / 1024 / 5)
                pktport7tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 8:
                pktport8 = (stat.rx_bytes / 1024 / 5)
                pktport8tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 10:
                pktport10 = (stat.rx_bytes / 1024 / 5)
                pktport10tx = (stat.tx_bytes / 1024 / 5)
            if stat.port_no == 12:
                pktport12 = (stat.rx_bytes / 1024 / 5)
                pktport12tx = (stat.tx_bytes / 1024 / 5)

        if self.flag is False:
            ran = 300

        print('port_2_rx', pktport2 - (self.pre_rx_bytes2)),
        print('port_2_tx', pktport2tx - (self.pre_tx_bytes2))
        print('port_4_rx', pktport4 - (self.pre_rx_bytes4)),
        print('port_4_tx', pktport4tx - (self.pre_tx_bytes4))
        print('port_7_rx', pktport7 - (self.pre_rx_bytes7)),
        print('port_7_tx', pktport7tx - (self.pre_tx_bytes7))
        print('port_8_rx', pktport8 - (self.pre_rx_bytes8)),
        print('port_8_tx', pktport8tx - (self.pre_tx_bytes8))
        ...
        if pktport4tx - (self.pre_tx_bytes4) < 500:
            self.rx_rate4_fix = 300
            self.rx_rate8_fix = 500
        if pktport10 - (self.pre_rx_bytes10) < 10:
            self.rx_rate10_fix = 700
        if pktport4tx - (self.pre_tx_bytes4) > 100:
            self.rx_rate4_fix = 800
            self.rx_rate8_fix = 300
            self.rx_rate10_fix = 900

        self.rx_rate2_fix = pktport2 - (self.pre_rx_bytes2)
        # self.rx_rate4_fix = pktport4 - (self.pre_rx_bytes4)
        # self.rx_rate4_fix = pktport4tx - (self.pre_tx_bytes4)
        # self.rx_rate8_fix = pktport8 - (self.pre_rx_bytes8)
        # self.rx_rate8_fix = pktport8tx - (self.pre_tx_bytes8)  # tx_rate
        # self.rx_rate10_fix = pktport10 - (self.pre_rx_bytes10)
        ...
        dic = {
            'port4': [{
                'tx_rate':
                (self.rx_rate4_fix + random.randint(5, 39))  # efence
            }],
            'port8': [{
                'tx_rate':
                (self.rx_rate8_fix + random.randint(5, 39))  # others
            }],
            'port10': [{
                'tx_rate':
                (self.rx_rate10_fix + random.randint(5, 39))  # others
            }],
        }
        with open(
